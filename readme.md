...
```
3. Ветки в GIT:
- git branch - просмотр всех локальных веток и текущей (отмечена звёздочкой в списке)
- git branch -a - просмотр ВСЕХ веток (текущая будет помечена remotes/origin/HEAD ->)
